function [result] = bfopenyokogawa(id, tile,varargin)
...
% X = omeMetadata.getPlanePositionX(tile-1,1).value.doubleValue/omeMetadata.getPixelsPhysicalSizeX(0).value.doubleValue;
% Y = omeMetadata.getPlanePositionY(tile-1,1).value.doubleValue/omeMetadata.getPixelsPhysicalSizeY(0).value.doubleValue;
...
numSeries = tile;%r.getSeriesCount();
result = cell(numSeries, 2);

globalMetadata = r.getGlobalMetadata();

for s = tile:tile%1:numSeries
